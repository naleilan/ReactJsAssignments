...
function reducer(state, action) {
  if (!state.isActive && action.type !== ACTIONS.OPEN_ACC) return state;
...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <p>
        <button
          onClick={() => dispatch({ type: ACTIONS.PAYlOAN })}
          disabled={!isActive}
        >
          Pay loan
        </button>
      ...
      <p>
        <button
          onClick={() => dispatch({ type: ACTIONS.CLOSE_ACC })}
          disabled={!isActive}
        >
          Close account
        </button>
